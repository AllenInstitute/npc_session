...
import re
import typing
from typing import Any, ClassVar, Optional, Protocol, Type, TypeAlias, TypeVar, Union
...
class MetadataRecord:
    ...
    def __init__(self, id: str | int, *args, **kwargs) -> None:
        self.id = id

    # def __new__(cls, *args, **kwargs) -> Self:
    #     """Initialize a Record object of the appropriate type.
    #     """
    #     if cls is __class__:
    ...
    # @staticmethod
    # def subclass_from_factory(*args, **kwargs) -> Type[MetadataRecord]:
    #     if isinstance(args[0], __class__):
    #         return args[0].__class__
    #     parsed = __class__.parse_id(args[0])
    #     if isinstance(parsed, str):
    #         return StrRecord
    #     if isinstance(parsed, int):
    #         return IntRecord
    #     raise TypeError(
    #         f'Could not find an appropriate {__class__.__name__} subclass for {args} {kwargs}'
    #     )
        
    @classmethod
    def parse_id(cls, value: id_input) -> id_type:
        ...
        # id_type = typing.get_args(cls.id_type) or cls.id_type
        # if not isinstance(id_type, tuple):
        #     id_type = (id_type,)
        if not isinstance(value, cls.id_type): # pragma: no cover
            raise TypeError(
                f"{cls.__name__}.id must be in {cls.id_type}, not {type(value)}"
            )
        return value
...
class StrRecord(MetadataRecord, str):
    id: str
    id_type: ClassVar[Type] = str
    def __init__(self, id: str | int, *args, **kwargs) -> None:
        MetadataRecord.__init__(self, id, *args, **kwargs)

class IntRecord(MetadataRecord, int):
    id: int
    id_type: ClassVar[Type] = int
...
class DatetimeRecord(StrRecord):
    ...
    def parse_id(cls, value: id_input) -> id_type:
        ...
        if date is None or time is None:
            raise ValueError(
                f"{cls.__name__} requires a date YYYY-MM-DD HH:MM:SS with optional separators, not {value!r}"
            )
        return str(super().parse_id(f"{date} {time}"))
...
class SessionRecord(StrRecord):
    ...
    To modify this, the class attribute `display_null_idx` should be set to True,
    and will be set for the session:
    >>> SessionRecord.display_null_idx = True
    >>> SessionRecord('366122_2022-04-25')
    '366122_2022-04-25_0'
    >>> SessionRecord.display_null_idx = False
    ...
    def with_idx(self, idx: int) -> SessionRecord:
        """Return new instance with specified idx.
        ...
        >>> SessionRecord.display_null_idx
        False
